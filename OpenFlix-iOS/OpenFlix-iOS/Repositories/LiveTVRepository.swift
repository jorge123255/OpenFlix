...
class LiveTVRepository: ObservableObject {
    ...
    func getChannelStream(id: String) async throws -> URL {
        ...
        // Fallback: use proxied stream URL through our server
        if let url = await api.channelStreamURL(id: id) {
            return url
        }

        // Last resort: fetch URL from API response
    ...
    func getGuide(start: Date? = nil, end: Date? = nil) async throws -> [ChannelWithPrograms] {
        ...
        let response = try await api.getGuide(start: start.map { formatter.string(from: $0) }, end: end.map { formatter.string(from: $0) })

        // Build debug info
        let programsMapCount = response.programs?.count ?? 0
        let programsMapKeys = response.programs?.keys.prefix(3).joined(separator: ", ") ?? "none"
        lastGuideDebug = "P:\(programsMapCount) keys:[\(programsMapKeys)] ch:\(response.allChannels.count) local:\(channels.count)"
        NSLog("GUIDE: %@", lastGuideDebug)

        var result: [ChannelWithPrograms] = []

        if !response.allChannels.isEmpty {
            // Server returned channels — use them directly
            for channelDTO in response.allChannels {
                let channel = Channel(
                    id: channelDTO.safeId,
                    channelId: channelDTO.channelId ?? channelDTO.tvgId,
            ...
            // Server returned only programs map (no channels array) — match against loaded channels
            let programsMap = response.programs ?? [:]
            let mapKeys = Array(programsMap.keys.prefix(3))
            let firstCh = channels.first
            NSLog("GUIDE MATCH: map has %d keys (e.g. %@), first channel id=%@ channelId=%@",
                  programsMap.count, mapKeys.joined(separator: ","),
                  firstCh?.id ?? "nil", firstCh?.channelId ?? "nil")

            for channel in channels {
                // Try matching by channelId (EPG/guide key) first
                var programs: [Program] = []
                if let channelId = channel.channelId {
                    programs = (programsMap[channelId] ?? []).map { $0.toDomain() }
                }

                // Try matching by DB id
                if programs.isEmpty {
                    programs = (programsMap[channel.id] ?? []).map { $0.toDomain() }
                }

                // Also try channel number as string key (server often keys by number)
                if programs.isEmpty, let number = channel.number {
                    programs = (programsMap[String(number)] ?? []).map { $0.toDomain() }
                }

                // Also try sourceId if available
                if programs.isEmpty, let sourceId = channel.sourceId {
                    programs = (programsMap[sourceId] ?? []).map { $0.toDomain() }
                }
    ...
    func getChannelGuide(channelId: String, start: Date? = nil, end: Date? = nil) async throws -> [Program] {
        let formatter = ISO8601DateFormatter()
        let response: GuideResponse = try await api.request(.getChannelGuide(channelId: channelId, start: start.map { formatter.string(from: $0) }, end: end.map { formatter.string(from: $0) }))
