...
class TVShowsViewModel: ObservableObject {
    ...
    private func loadRecentlyAdded() async {
        ...
        do {
            ...
            recentlyAdded = deduplicateShows(recent)
        } catch {
    ...
    /// Deduplicate shows by normalized name, keeping the best representative item
    private func deduplicateShows(_ items: [MediaItem]) -> [MediaItem] {
        var seen: [String: MediaItem] = [:]
        var order: [String] = []

        for item in items {
            let showName = item.grandparentTitle ?? item.parentTitle ?? item.title
            let key = normalizeShowName(showName)

            if let existing = seen[key] {
                // Prefer items with artwork
                if existing.thumb == nil && item.thumb != nil {
                    seen[key] = item
                } else if existing.art == nil && item.art != nil {
                    seen[key] = item
                }
            } else {
                seen[key] = item
    ...
    func loadMoreGridItems() async {
        ...
        do {
            ...
            let newItems = deduplicateShows(allShows + result.items)
            allShows = newItems
