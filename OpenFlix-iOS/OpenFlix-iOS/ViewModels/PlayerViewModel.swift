...
class PlayerViewModel: ObservableObject {
    ...
    // Commercial skip
    @Published var commercials: [Commercial] = []
    @Published var isInCommercial = false
    @Published var currentCommercial: Commercial?
    var commercialSkipEnabled: Bool {
        UserDefaults.standard.bool(forKey: "commercial_skip_enabled")
    }
    private var lastSkippedCommercialId: String?

    // Skip markers (intro/credits)
    @Published var skipMarkers: [SkipMarkerDTO] = []
    @Published var activeSkipMarker: SkipMarkerDTO?
    @Published var showSkipButton = false
    @AppStorage("auto_skip_intro") var autoSkipIntro = false
    @AppStorage("auto_skip_credits") var autoSkipCredits = false
    private var lastSkippedMarkerId: String?
    ...
    // MARK: - Commercial Skip

    func setCommercials(_ segments: [Commercial]) {
        commercials = segments
        logger.info("Loaded \(segments.count) commercial segments")
    }

    func skipCurrentCommercial() {
        guard let commercial = currentCommercial else { return }
        let skipToSeconds = Double(commercial.end) / 1000.0
        logger.info("Skipping commercial to \(skipToSeconds)s")
        seek(to: skipToSeconds)
        lastSkippedCommercialId = commercial.id
        isInCommercial = false
        currentCommercial = nil
    }

    private func checkCommercials(at timeSeconds: Double) {
        guard !commercials.isEmpty else { return }
        let timeMs = Int(timeSeconds * 1000)

        // Find if we're inside a commercial
        if let commercial = commercials.first(where: { timeMs >= $0.start && timeMs < $0.end }) {
            if !isInCommercial || currentCommercial?.id != commercial.id {
                isInCommercial = true
                currentCommercial = commercial

                // Auto-skip if enabled and we haven't already skipped this one
                if commercialSkipEnabled && lastSkippedCommercialId != commercial.id {
                    logger.info("Auto-skipping commercial at \(timeSeconds)s")
                    skipCurrentCommercial()
                }
            }
        } else {
            if isInCommercial {
    ...
    // MARK: - Skip Markers (Intro/Credits)

    func loadSkipMarkers(for mediaId: String) async {
        let api = OpenFlixAPI.shared
        do {
            let response = try await api.getSkipMarkers(id: mediaId)
            skipMarkers = response.markers
            logger.info("Loaded \(response.markers.count) skip markers")
        } catch {
            logger.info("No skip markers available: \(error.localizedDescription)")
        }
    }

    func skipActiveMarker() {
        guard let marker = activeSkipMarker else { return }
        let skipTo = marker.endTime
        logger.info("Skipping \(marker.type) to \(skipTo)s")
        seek(to: skipTo)
        lastSkippedMarkerId = marker.id?.stringValue
        showSkipButton = false
        activeSkipMarker = nil
    }

    private func checkSkipMarkers(at timeSeconds: Double) {
        guard !skipMarkers.isEmpty else { return }

        if let marker = skipMarkers.first(where: { timeSeconds >= $0.startTime && timeSeconds < $0.endTime }) {
            let markerId = marker.id?.stringValue ?? "\(marker.type)-\(marker.startTime)"
            if activeSkipMarker == nil || lastSkippedMarkerId != markerId {
                activeSkipMarker = marker
                showSkipButton = true

                // Auto-skip if user preference is set
                let shouldAutoSkip = (marker.type == "intro" && autoSkipIntro) ||
                                     (marker.type == "credits" && autoSkipCredits)
                if shouldAutoSkip && lastSkippedMarkerId != markerId {
                    skipActiveMarker()
                }
            }
        } else {
    ...
    func loadMedia(_ item: MediaItem) async {
        ...
        do {
            // If mediaVersions is empty and no direct streamUrl, fetch full details first
            var mediaToPlay = item
            if item.mediaVersions.isEmpty && item.streamUrl == nil {
            ...
            // Verify we have playable media — M3U VOD items use streamUrl instead of mediaVersions
            guard !mediaToPlay.mediaVersions.isEmpty || mediaToPlay.streamUrl != nil else {
            ...
            // Server handles offset in HLS playlist, so no client-side seek needed
            ...
            // Load skip markers (intro/credits) for library media
            await loadSkipMarkers(for: String(mediaToPlay.id))
    ...
    private func setupPlayer(url: URL, startPosition: Int?) async {
        ...
        // Configure audio session for playback (required on iOS for speaker output)
        do {
            ...
            DispatchQueue.main.async {
                ...
                case .readyToPlay:
                    ...
                    if let position = startPosition {
                        if position == -1 {
                            // Seek to live edge (near end of available content)
                            let duration = item.duration
                            if duration.isNumeric && duration.seconds > 10 {
                                let liveEdge = CMTime(seconds: duration.seconds - 5, preferredTimescale: 1000)
                                self.player?.seek(to: liveEdge)
                            }
                        } else if position > 0 {
        ...
        timeObserver = player?.addPeriodicTimeObserver(forInterval: interval, queue: .main) { [weak self] time in
            Task { @MainActor in
                ...
                self?.checkCommercials(at: seconds)
                self?.checkSkipMarkers(at: seconds)
            }
        }
