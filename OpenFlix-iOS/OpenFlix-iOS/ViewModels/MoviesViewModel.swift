...
class MoviesViewModel: ObservableObject {
    ...
    private var allMovieSectionIds: [Int] = []
    ...
    func loadMoviesHub() async {
        ...
        do {
            ...
            if let first = movieSections.first {
                movieSectionId = first.id
                allMovieSectionIds = movieSections.map { $0.id }
                NSLog("MoviesViewModel: Using %d movie sections: %@", movieSections.count, movieSections.map { "\($0.id) (\($0.title))" }.joined(separator: ", "))
            } else {
        ...
        // Load all data sequentially (avoids @MainActor + TaskGroup crash)
        await loadContinueWatching()
        await loadRecentlyAdded()
        await loadAllMoviesForGenres()
    ...
    private func loadRecentlyAdded() async {
        guard !allMovieSectionIds.isEmpty else {
            NSLog("MoviesViewModel: No section IDs for recently added")
            return
        }
        // Load from all movie sections and merge
        var allRecent: [MediaItem] = []
        for sectionId in allMovieSectionIds {
            let items = (try? await mediaRepository.getSectionRecentlyAdded(sectionId: sectionId)) ?? []
            allRecent.append(contentsOf: items)
        }
        // Sort by addedAt descending, dedup by title+year
        allRecent.sort { ($0.addedAt ?? .distantPast) > ($1.addedAt ?? .distantPast) }
        var seen = Set<String>()
        recentlyAdded = allRecent.filter { item in
            let key = item.title.lowercased() + "-" + String(item.year ?? 0)
            return seen.insert(key).inserted
        }
        NSLog("MoviesViewModel: Loaded %d recently added movies (from %d sections)", recentlyAdded.count, allMovieSectionIds.count)
    }

    // MARK: - All Movies (for genre extraction)

    private func loadAllMoviesForGenres() async {
        guard !allMovieSectionIds.isEmpty else { return }
        ...
        // Dedup by title+year, keep first occurrence
        var seen = Set<String>()
        allMovies = combined.filter { item in
            let key = item.title.lowercased() + "-" + String(item.year ?? 0)
            return seen.insert(key).inserted
        }
        hasMore = combined.count < totalSize
