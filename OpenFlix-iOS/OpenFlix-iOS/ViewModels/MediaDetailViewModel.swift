...
class MediaDetailViewModel: ObservableObject {
    ...
    func selectSeason(_ season: MediaItem) async {
        ...
        NSLog("MediaDetailViewModel: selectSeason id=%d index=%d title='%@'", season.id, season.index ?? -1, season.title)

        do {
            episodes = try await mediaRepository.getMediaChildren(id: season.id)
            episodes.sort { ($0.index ?? 0) < ($1.index ?? 0) }
            NSLog("MediaDetailViewModel: Loaded %d episodes for season %d", episodes.count, season.index ?? -1)
        } catch {
            NSLog("MediaDetailViewModel: Failed to load episodes: %@", error.localizedDescription)
            episodes = []
        }
    }
    ...
    var canPlay: Bool {
        ...
        if item.streamUrl != nil { return true }
