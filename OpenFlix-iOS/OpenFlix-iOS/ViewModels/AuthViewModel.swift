...
class AuthViewModel: ObservableObject {
    ...
    func checkAuthStatus() async {
        ...
        // Save the token before the status check (it may be cleared on network failure)
        let savedToken = KeychainHelper.shared.getToken()
        var hasValidToken = await authRepository.checkAuthStatus()

        // If token check failed and we have a saved token + machineId, the server may be
