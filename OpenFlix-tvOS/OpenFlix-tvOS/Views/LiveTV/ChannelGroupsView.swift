...
struct ChannelGroupsView: View {
    ...
    @State private var editingGroup: UIChannelGroup?
...
struct UIChannelGroupCard: View {
    let group: UIChannelGroup
    let onTap: () -> Void
    let onEdit: () -> Void
    let onDelete: () -> Void
...
struct UIChannelGroup: Identifiable {
    let id: String
    let name: String
    let icon: String
...
class ChannelGroupsViewModel: ObservableObject {
    @Published var groups: [UIChannelGroup] = []
    ...
    func deleteGroup(_ group: UIChannelGroup) {
        groups.removeAll { $0.id == group.id }
        // TODO: Delete via API
    }
    
    func playGroup(_ group: UIChannelGroup) {
